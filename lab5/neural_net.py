...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        return s(dotP([i.get_value() for i in self.my_weights],[i.output() for i in self.my_inputs]))
    ...
    def compute_doutdx(self, elem):
        ...
        i=indexSafe(self.my_weights,elem)
        if i!=None:
            #d(y_D)/d(w_BD) = d(y_D)/dz * dz/dw_BD.
            #d(y_D)/dz = y_D*(1-y_D) and dz/dw_BD = y_B.
            return self.output()*(1-self.output()) * self.my_inputs[i].output()
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        # P(o) = -0.5 (d - o)**2
        return -0.5 * (self.my_desired_val - self.my_input.output())
